      ...
      <div class="span12">
        ...
        <div class="widget-box">
          ...
          <div class="widget-content nopadding">
            @if(isset($user))
              <form action="{{Request::root()}}/admin/user/add-new-user" method="post" class="form-horizontal" enctype="multipart/form-data" name="basic_validate" id="basic_validate" novalidate="novalidate">
            ...
            @endif
                <div class="control-group @if ($errors->has('name')) error @endif">
                  ...
                  <div class="controls">
                    @if(isset($user))
                    <input type="text" name="name" id="name" value="@if($errors->has('name')){{old('name')}}@else{{$user->name}}@endif ">
                      @if ($errors->has('name'))
                ...
                <div class="control-group @if ($errors->has('email')) error @endif">
                  ...
                  <div class="controls">
                    @if(isset($user))
                      <input type="text" name="email" id="email" value="@if($errors->has('email')){{old('email')}}@else{{$user->email}}@endif">
                      @if ($errors->has('email'))
                          <span for="email" generated="true" class="help-inline">
                            {{ $errors->first('email') }}
                          </span>
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                    @if(isset($user))
                    <input type="text" data-date="{{date('d/m/Y')}}" data-date-format="dd/mm/yyyy" value="@if($errors->has('dob')){{old('dob')}}@else{{$user->dob}}@endif" placeholder="{{date('d/m/Y')}}" class="datepicker" name="dob" id="date">
                    @else
